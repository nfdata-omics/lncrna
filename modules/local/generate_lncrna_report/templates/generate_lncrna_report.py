...
'''
    ...
    with open(args.summary, 'w') as f:
        NL = "\n"
        f.write("=== LncRNA Pipeline Summary ===" + NL + NL)
        f.write(f"Total genes in final GTF: {gtf_stats.get('total_genes', 'N/A')}" + NL)
        f.write(f"LncRNA classifications: {len(class_df)}" + NL + NL)
        if de_stats:
            f.write("Differential Expression Results:" + NL)
            f.write(f"  Total genes analyzed: {de_stats.get('total', 'N/A')}" + NL)
            f.write(f"  Differentially expressed genes: {de_stats.get('significant', 'N/A')}" + NL)
            f.write(f"    - Upregulated: {de_stats.get('upregulated', 'N/A')}" + NL)
            f.write(f"    - Downregulated: {de_stats.get('downregulated', 'N/A')}" + NL)
            f.write(f"  Differentially expressed lncRNAs: {de_stats.get('lncrna_de', 'N/A')}" + NL + NL)
        else:
            f.write("Differential expression analysis: Not performed" + NL + NL)
        f.write("LncRNA Classification:" + NL)
        if not class_stats_df.empty:
            if class_stats_df.shape[1] >= 2:
                for _, row in class_stats_df.iterrows():
                    f.write(f"  - {row.iloc[0]}: {row.iloc[1]}" + NL)
            elif class_stats_df.shape[1] == 1:
                for _, row in class_stats_df.iterrows():
                    f.write(f"  - {row.iloc[0]}" + NL)
        else:
            f.write("  No classification statistics available." + NL)
    # Write JSON stats
    stats_json = {
        'pipeline': 'lncrna',
        ...
        'classification_stats': class_stats_df.to_dict('records') if not class_stats_df.empty else []
    }
...
if __name__ == '__main__':
    if len(sys.argv) == 1:
        prefix_value = "${task.ext.prefix}" if "${task.ext.prefix}" != "null" else "lncRNA_pipeline"
        de_results_arg = "$de_results" if "$de_results" != "NO_FILE" else None
        de_plots_arg = "$de_plots" if "$de_plots" != "NO_FILE" else None
        count_summary_arg = "$count_summary" if "$count_summary" != "NO_FILE" else None
        rename_mapping_arg = "$rename_mapping" if "$rename_mapping" != "NO_FILE" else None
        cpat_arg = "$cpat_comparison" if "$cpat_comparison" != "NO_FILE" else "NO_FILE"
        ...
        argv = [
            ...
            "$final_gtf",
            "--output",
            f"{prefix_value}.final_report.html",
            "--summary",
            f"{prefix_value}.summary.txt",
        ]
        if de_results_arg:
            argv.extend(["--de_results", de_results_arg])
        if de_plots_arg:
            argv.extend(["--de_plots", de_plots_arg])
        if count_summary_arg:
            argv.extend(["--count_summary", count_summary_arg])
        if rename_mapping_arg:
            argv.extend(["--rename_mapping", rename_mapping_arg])
