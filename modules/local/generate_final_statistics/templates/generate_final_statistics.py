...
def parse_cpat_results(cpat_file):
    return pd.read_csv(cpat_file, sep='\t')


def parse_classification(class_file):
    return pd.read_csv(class_file, sep='\t')


def create_cpat_comparison_plot(cpat_lncrna_df, cpat_coding_df, output_file):
    cpat_lncrna_df = cpat_lncrna_df.copy()
    cpat_coding_df = cpat_coding_df.copy()
...
def create_classification_barplot(class_df, output_file):
    ...
    plt.title('LncRNA Classification by Genomic Location', fontsize=14)
    plt.xticks(rotation=45, ha='right')

    for i, v in enumerate(category_counts):
        ax.text(i, v + max(category_counts) * 0.02, str(v), ha='center', va='bottom')
...
def main():
    ...
    total_genes, total_transcripts = parse_gtf(final_gtf)
    lncrna_genes, lncrna_transcripts = parse_gtf(lncrna_gtf)

    print("Parsing CPAT results...", file=sys.stderr)
    cpat_lncrna_df = parse_cpat_results(cpat_lncrna)
    cpat_coding_df = parse_cpat_results(cpat_coding)

    print("Parsing classification...", file=sys.stderr)
    class_df = parse_classification(classification)
    ...
    create_cpat_comparison_plot(cpat_lncrna_df, cpat_coding_df, f"{prefix}.cpat_comparison.png")
    create_classification_barplot(class_df, f"{prefix}.classification_barplot.png")

    print("Generating HTML report...", file=sys.stderr)
    generate_html_report(stats_dict, prefix)
...
if __name__ == '__main__':
    ...
    versions_this_module["${task.process}"] = {"python": platform.python_version(), "matplotlib": matplotlib.__version__}
